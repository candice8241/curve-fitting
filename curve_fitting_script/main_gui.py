...
# Fixed import: changed 'prefernce' to 'gui_components'
from gui_components import GUIBase, ModernButton, ModernTab, CuteSheepProgressBar

# Import modules (these will need to be created separately)
try:
    from powder_module import PowderXRDModule
except ImportError:
    PowderXRDModule = None

try:
    from radial_module import RadialXRDModule
except ImportError:
    RadialXRDModule = None

try:
    from single_crystal_module import SingleCrystalModule
except ImportError:
    SingleCrystalModule = None
...
class XRDProcessingGUI(GUIBase):
    ...
    def __init__(self, root):
        ...
        # Try to set icon (update path as needed)
        try:
    ...
    def switch_tab(self, tab_name):
        ...
        if tab_name == "powder":
            if PowderXRDModule is None:
                self._show_module_not_found("Powder XRD")
            else:
            ...
            if RadialXRDModule is None:
                self._show_module_not_found("Radial XRD")
            else:
            ...
            if SingleCrystalModule is None:
                self._show_module_not_found("Single Crystal")
            else:
    ...
    def _show_module_not_found(self, module_name):
        """
        Display message when module is not available

        Args:
            module_name: Name of the missing module
        """
        card = self.create_card_frame(self.scrollable_frame)
        card.pack(fill=tk.BOTH, expand=True, pady=20)

        tk.Label(card, text="📦", bg=self.colors['card_bg'],
                font=('Segoe UI Emoji', 48)).pack(pady=(40, 20))

        tk.Label(card, text=f"{module_name} Module Not Found",
                bg=self.colors['card_bg'], fg=self.colors['text_dark'],
                font=('Comic Sans MS', 16, 'bold')).pack(pady=(0, 10))

        tk.Label(card, text=f"The {module_name.lower()}_module.py file is not available.\nPlease ensure all required modules are installed.",
                bg=self.colors['card_bg'], fg=self.colors['text_light'],
                font=('Comic Sans MS', 11), justify=tk.CENTER).pack(pady=(0, 40))
...
def launch_main_app():
    ...
    # Set window icon (update path as needed)
    icon_path = r"D:\HEPS\ID31\dioptas_data\github_felicity\batch\ChatGPT Image.ico"
    if os.path.exists(icon_path):
        try:
    ...
    welcome_text = ("💜 Hey there, crystal cutie! Ready to sparkle your XRD data? 🌈\n"
                    "\n"
                    "\n"
                    "📧 Contact: candicewang928@gmail.com")  # Fixed typo: egmail -> gmail
    label = tk.Label(
        root,
        text=welcome_text,
        font=cute_font,
