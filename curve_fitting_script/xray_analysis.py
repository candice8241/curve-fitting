...
def select_crystal_system():
    """
    Interactive function for user to select crystal system

    Returns:
        selected_system: Crystal system key (e.g., 'cubic_fcc')
    """
    print("\n" + "="*60)
    print("Available Crystal Systems:")
    print("="*60)

    systems_list = list(CRYSTAL_SYSTEMS.keys())
    for i, key in enumerate(systems_list, 1):
        system_info = CRYSTAL_SYSTEMS[key]
        print(f"{i}. {system_info['name']} ({key})")
        print(f"   - Minimum peaks required: {system_info['min_peaks']}")
        print(f"   - Atoms per unit cell: {system_info['atoms_per_cell']}")

    print("="*60)

    while True:
        try:
            choice = int(input(f"Select crystal system (1-{len(systems_list)}): "))
            if 1 <= choice <= len(systems_list):
                selected_system = systems_list[choice - 1]
                print(f"\n✓ Selected: {CRYSTAL_SYSTEMS[selected_system]['name']}")
                return selected_system
            else:
                print(f"Please enter a number between 1 and {len(systems_list)}")
        except ValueError:
...
def main():
    ...
    print("\n[1] Select crystal system for ORIGINAL PEAKS (before transition):")
    original_system = select_crystal_system()

    print("\n[2] Select crystal system for NEW PEAKS (after transition):")
    new_system = select_crystal_system()
