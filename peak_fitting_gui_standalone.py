# -*- coding: utf-8 -*-
"""
Interactive Peak Fitting with GUI - Complete Standalone Version
@author: candicewang928@gmail.com

This is a complete, self-contained version with all functionality in one file.
...
class PeakFittingGUI:
    ...
    def __init__(self, master):
        self.master = master
        self.master.title("Interactive XRD Peak Fitting Tool - Complete Version")
        ...
        if self.selecting_bg:
            if event.button == 1:  # Left click - add point
                ...
                if len(self.bg_points) > 0:
                    ...
                    delete_idx = np.argmin(point_distances)
    ...
    def auto_find_peaks(self):
        ...
        try:
            ...
            if len(peaks) == 0:
                messagebox.showinfo("No Peaks Found", "No significant peaks detected.")
                return
    ...
    def toggle_overlap_mode(self):
        ...
        if self.overlap_mode:
            ...
            self.update_info("Overlap mode ON: Peaks within 3.5*FWHM grouped\n")
        else:
        ...
        try:
            ...
            if len(self.bg_points) >= 2:
                ...
                global_bg, global_bg_points = fit_global_background(
                    self.x, self.y, sorted_peaks, method=bg_method
                )
                self.update_info(f"Piecewise background: {len(global_bg_points)} anchor points\n")
            ...
            for group in peak_groups:
                if len(group) > 1:
                    ...
                    self.update_info(f"Peaks {original_nums} grouped\n")
    ...
    def quick_save_results(self):
        """Quickly save to source file directory"""
        if self.fit_results is None:
            messagebox.showwarning("No Results", "Please fit peaks before saving!")
            return
...
if __name__ == "__main__":
    print("=" * 60)
    print("XRD Peak Fitting Tool - Complete Standalone Version")
    print("Version 2.0.0")
