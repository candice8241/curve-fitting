...
class XRayDiffractionAnalyzer:
    ...
    def __init__(self, wavelength=0.4133, peak_tolerance_1=0.3,
        ...
        Parameters:
            ...
            peak_tolerance_1 (float): Tolerance for phase transition identification (degrees)
            peak_tolerance_2 (float): Tolerance for determining new peak count (degrees)
            peak_tolerance_3 (float): Tolerance for tracking new peaks at subsequent pressures (degrees)
    ...
    def _export_peaks_to_csv(peaks_dict, output_path, dataset_type="Peaks"):
        """
        Export peaks dataset to CSV file in a structured format.
        ...
        # Find the maximum number of peaks across all pressures
        max_peaks = max(len(peaks) for peaks in peaks_dict.values())

        # Create column headers
        columns = ['Pressure (GPa)', 'Number of Peaks']
        for i in range(max_peaks):
            columns.append(f'Peak_{i+1}_2Theta')

        # Build data rows
        data_rows = []
        for pressure in sorted(peaks_dict.keys()):
            peaks = peaks_dict[pressure]
            row = [pressure, len(peaks)]

            # Add peak positions
            for peak in peaks:
                row.append(peak)

            # Fill remaining columns with NaN if fewer peaks
...
if __name__ == "__main__":
    ...
    analyzer = XRayDiffractionAnalyzer(
        ...
        peak_tolerance_2=0.4,
        peak_tolerance_3=0.01,
        n_pressure_points=4
    )
