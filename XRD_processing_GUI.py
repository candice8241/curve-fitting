...
class XRDProcessingGUI:
    ...
    def _separate_peaks_thread(self):
        try:
            ...
            input_csv = self.phase_peak_csv.get()
            ...
            self.log(f"📁 Input file: {input_csv}")
            self.log(f"📊 Using tolerances: Tol-1={tolerance_1}, Tol-2={tolerance_2}, Tol-3={tolerance_3}")
            self.log(f"📊 N pressure points: {n_points}")

            # Create analyzer instance
            analyzer = XRayDiffractionAnalyzer(
                csv_file=input_csv,
                crystal_system='FCC',  # Default, will be updated later if needed
                wavelength=self.phase_wavelength.get()
            )

            # Collect tracked new peaks
            self.log("🔍 Collecting tracked new peaks...")
            new_peaks_df = analyzer.collect_tracked_new_peaks(
                tolerance_range_1=tolerance_1,
                tolerance_range_2=tolerance_2,
                tolerance_intensity=tolerance_3,
                n_pressure_points=n_points
            )

            # Save new peaks dataset
            new_peaks_output = output_dir / f"{base_filename}_new_peaks_dataset.csv"
            new_peaks_df.to_csv(new_peaks_output, index=False)
            self.log(f"✅ New peaks saved to: {new_peaks_output.name}")

            # Build original peak dataset
            self.log("🔍 Building original peak dataset...")
            original_peaks_df = analyzer.build_original_peak_dataset(new_peaks_df)

            # Save original peaks dataset
            original_peaks_output = output_dir / f"{base_filename}_original_peaks_dataset.csv"
            original_peaks_df.to_csv(original_peaks_output, index=False)
            self.log(f"✅ Original peaks saved to: {original_peaks_output.name}")

            self.log(f"📊 Total new peaks found: {len(new_peaks_df)}")
            self.log(f"📊 Total original peaks: {len(original_peaks_df)}")
    ...
    def run_phase_analysis(self):
        if not self.phase_volume_csv.get() or not self.phase_volume_output.get():
            messagebox.showerror("Error", "Please fill all required fields for volume calculation")
            return
    ...
    def _run_phase_analysis_thread(self):
        try:
            self.progress.start()
            self.log("🔬 Starting Phase Analysis (Volume Calculation)")

            analyzer = XRayDiffractionAnalyzer(
                csv_file=self.phase_volume_csv.get(),
                crystal_system=self.phase_volume_system.get(),
                wavelength=self.phase_wavelength.get()
            )

            # Calculate volumes
            self.log("📐 Calculating unit cell volumes...")
            results_df = analyzer.calculate_volumes()

            # Save results
            output_file = Path(self.phase_volume_output.get()) / f"volume_results_{Path(self.phase_volume_csv.get()).stem}.csv"
