...
class XRDProcessingGUI:
    ...
    def _run_birch_murnaghan_thread(self):
        ...
        try:
            ...
            # FIXED: Try multiple parameter name variations for BirchMurnaghanFitter initialization
            # Common parameter names: input_file, csv_file, data_file, filename
            ...
            # Attempt 1: Try with input_file parameter
            try:
                ...
                self.log("✓ Initialized with 'input_file' parameter")
            except TypeError as e:
                initialization_error = e

                # Attempt 2: Try with csv_file parameter
                try:
                    fitter = BirchMurnaghanFitter(
                    ...
                    self.log("✓ Initialized with 'csv_file' parameter")
                except TypeError:

                    # Attempt 3: Try with filename parameter
                    try:
                        fitter = BirchMurnaghanFitter(
                        ...
                        self.log("✓ Initialized with 'filename' parameter")
                    except TypeError:

                        # Attempt 4: Try with positional arguments
                        try:
                            fitter = BirchMurnaghanFitter(
                                input_file_path,
                                output_directory,
                            ...
                            self.log("✓ Initialized with positional arguments")
                        except TypeError:

                            # Attempt 5: Try with file_path parameter
                            try:
                                fitter = BirchMurnaghanFitter(
                                ...
                                self.log("✓ Initialized with 'file_path' parameter")
                            except TypeError:
                                # All attempts failed, raise the original error
                                raise initialization_error

            if fitter is None:
                raise ValueError("Failed to initialize BirchMurnaghanFitter with any known parameter combination")

            # Call fit method (with or without initial guess depending on what the class expects)
            try:
                results = fitter.fit()
            except TypeError:
                # If fit() requires parameters, try with automatic initial guess
                results = fitter.fit(initial_guess=None)
            ...
            self.log(f"💾 Results saved to: {output_directory}")
            ...
            self.log(f"❌ Error: {str(e)}")
            self.log(f"Details:\n{error_details}")
            messagebox.showerror("Error", f"BM fitting failed:\n{str(e)}\n\nPlease check that the BirchMurnaghanFitter class is properly installed and configured.")
        finally:
            self.progress.stop()
