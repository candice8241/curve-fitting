...
class XRDProcessingGUI:
    ...
    def _run_birch_murnaghan_thread(self):
        ...
        try:
            ...
            csv_path = os.path.join(output_directory, f'BM_{order}rd_order_parameters.csv')
            results_df = pd.DataFrame([{
                'Fitting Order': f'{order}rd Order' if order == 3 else '2nd Order',
                'V₀ (Å³/atom)': f"{fit_results['V0']:.6f}",
                'V₀ Error': f"{fit_results['V0_err']:.6f}",
                'B₀ (GPa)': f"{fit_results['B0']:.4f}",
                'B₀ Error': f"{fit_results['B0_err']:.4f}",
                "B₀'": f"{fit_results['B0_prime']:.6f}",
                "B₀' Error": f"{fit_results['B0_prime_err']:.6f}",
                'R²': f"{fit_results['R_squared']:.8f}",
                'RMSE (GPa)': f"{fit_results['RMSE']:.6f}"
            }])
            results_df.to_csv(csv_path, index=False, encoding='utf-8-sig')
            self.log(f"💾 Parameters saved: {os.path.basename(csv_path)}")
            ...
            self.log(f"   - {os.path.basename(csv_path)} : Fitting parameters table")
            self.log(f"{'='*60}\n")
