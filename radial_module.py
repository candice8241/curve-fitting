...
class RadialXRDModule(GUIBase):
    ...
    def _run_single_sector(self, params):
        ...
        self.radial_log(f"📁 PONI file: {os.path.basename(params['poni_path'])}")
        if params['mask_path']:
            self.radial_log(f"🎭 Mask file: {os.path.basename(params['mask_path'])}")
        ...
        self.radial_log(f"📁 Output directory: {params['output_dir']}")
    ...
    def _run_multiple_sectors(self, params):
        ...
        if 'preset_name' in params:
            self.radial_log(f"📐 Using preset: {params['preset_name']}")
        else:
    ...
    def _integrate_sector(self, params, azim_start, azim_end, sector_label):
        ...
        for idx, h5_file in enumerate(input_files):
            ...
            np.savetxt(output_path, np.column_stack([result.radial, result.intensity]),
                      header=f"{params['unit']}  Intensity", comments='#')
