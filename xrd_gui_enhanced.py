...
class XRDProcessingGUI:
    ...
    def setup_powder_content(self):
        ...
        self.create_tiny_folder_picker(vol_content, "Input Directory (Peak Files)", self.volume_input_dir)
        ...
        lattice_combo = ttk.Combobox(lattice_container, textvariable=self.lattice_type,
                                    values=['cubic', 'tetragonal', 'hexagonal', 'orthorhombic'],
        ...
        k0p_entry = tk.Entry(k0p_container, textvariable=self.k0_prime_guess,
                            font=('Comic Sans MS', 9), width=15)
        k0p_entry.pack(anchor=tk.W)
        ...
        row1 = tk.Frame(button_container, bg=self.colors['bg'])
        row1.pack(pady=(0, 10))

        ModernButton(row1, "Run Integration", self.run_integration, icon="🐿️",
                    bg_color=self.colors['secondary'], hover_color=self.colors['primary_hover'],
                    width=220, height=45).pack(side=tk.LEFT, padx=8)

        ModernButton(row1, "Run Fitting", self.run_fitting, icon="🐻",
                    bg_color=self.colors['secondary'], hover_color=self.colors['primary_hover'],
                    width=220, height=45).pack(side=tk.LEFT, padx=8)

        ModernButton(row1, "Full Pipeline", self.run_full_pipeline, icon="🦔",
                    bg_color=self.colors['primary'], hover_color=self.colors['accent'],
                    width=220, height=45).pack(side=tk.LEFT, padx=8)

        # Second row of buttons
        row2 = tk.Frame(button_container, bg=self.colors['bg'])
        row2.pack()

        ModernButton(row2, "Calculate Volumes", self.run_volume_calculation, icon="📊",
                    bg_color="#06D6A0", hover_color="#05B88A",
                    width=220, height=45).pack(side=tk.LEFT, padx=8)

        ModernButton(row2, "Birch-Murnaghan Fit", self.run_birch_murnaghan, icon="⚗️",
    ...
    def validate_volume_inputs(self):
        if not self.volume_input_dir.get():
            messagebox.showerror("Error", "Please select input directory containing peak files")
            return False
    ...
    def _run_volume_calculation_thread(self):
        try:
            ...
            analyzer = XRayDiffractionAnalyzer(
                folder_path=self.volume_input_dir.get(),
    ...
    def _run_birch_murnaghan_thread(self):
        try:
            self.progress.start()
            self.log("🎀 " * 15)
            self.log("⚗️ Starting Birch-Murnaghan Equation of State Fitting")
            ...
            self.show_cute_success("Birch-Murnaghan fitting completed successfully!")
