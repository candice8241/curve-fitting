...
Created: Nov 15, 2025
Modified: Nov 16, 2025 - Fixed UI jumping issue when deleting sectors
"""
...
# ============================================================================
# GUI Module with OPTIMIZED DELETE (NO JUMPING)
# ============================================================================

class AzimuthalIntegrationModule(GUIBase):
    """Azimuthal Integration module - OPTIMIZED DELETE (NO UI JUMPING)"""
    ...
    def _delete_sector(self, index):
        """Delete sector - OPTIMIZED (no UI jumping) 🎯"""
        if len(self.custom_sectors) <= 1:
            messagebox.showwarning("Warning", "At least one sector must be defined!")
            return

        # 🎯 Step 1: Ensure current layout is stable
        self.sectors_container.update_idletasks()

        # 🎯 Step 2: Remove from data model
        del self.custom_sectors[index]

        # 🎯 Step 3: Hide widget first (don't destroy yet)
        if index < len(self.sector_row_widgets):
            row_widget = self.sector_row_widgets[index]
            row_widget.pack_forget()  # Hide without destroying
            del self.sector_row_widgets[index]

            # 🎯 Step 4: Update numbering for remaining rows
            self._renumber_sectors()

            # 🎯 Step 5: Force layout update before destroying
            self.sectors_container.update_idletasks()

            # 🎯 Step 6: Now safely destroy the widget
            row_widget.destroy()
