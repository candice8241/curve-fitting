...
Created: Nov 15, 2025
Modified: Nov 16, 2025 - Performance Optimization (Fixed UI lag)
"""
...
# ============================================================================
# GUI Module with PERFORMANCE OPTIMIZATIONS
# ============================================================================

class AzimuthalIntegrationModule(GUIBase):
    """Azimuthal Integration module - PERFORMANCE OPTIMIZED VERSION"""
    ...
    def _create_number_spinner(self, parent, variable, min_val, max_val):
        """Create pagination-style number spinner"""
        container = tk.Frame(parent, bg=self.colors['card_bg'])

        dec_btn = tk.Button(container, text="<",
                           command=lambda: self._adjust_value(variable, -1, min_val, max_val),
                           bg='#E8D4F0', fg='#5A5A5A',
                           font=('Comic Sans MS', 8, 'bold'), relief='flat',
                           width=2, height=1, cursor='hand2',
                           borderwidth=1, highlightthickness=0)
        dec_btn.pack(side=tk.LEFT, padx=(0, 1))

        value_frame = tk.Frame(container, bg='white', relief='solid', borderwidth=1)
        value_frame.pack(side=tk.LEFT, padx=1)
        value_label = tk.Label(value_frame, textvariable=variable,
                              bg='white', fg='#333333',
                              font=('Comic Sans MS', 9), width=5,
                              padx=3, pady=2)
        value_label.pack()

        inc_btn = tk.Button(container, text=">",
                           command=lambda: self._adjust_value(variable, 1, min_val, max_val),
                           bg='#E8D4F0', fg='#5A5A5A',
                           font=('Comic Sans MS', 8, 'bold'), relief='flat',
                           width=2, height=1, cursor='hand2',
                           borderwidth=1, highlightthickness=0)
        inc_btn.pack(side=tk.LEFT, padx=(1, 0))

        return container

    def _adjust_value(self, variable, delta, min_val, max_val):
        """Adjust variable value within bounds"""
    ...
    def _create_run_button_section(self):
        """Run button directly on background - NO CARD, smaller size"""
    ...
    def _setup_single_sector_ui(self):
        """Single sector with custom spinners"""
        ...
        self._create_number_spinner(start_cont, self.azimuth_start, 0, 360).pack(anchor=tk.W)
        ...
        tk.Label(end_cont, text="End Angle (°)", bg=self.colors['card_bg'],
                fg=self.colors['text_dark'], font=('Comic Sans MS', 10, 'bold')).pack(anchor=tk.W, pady=(0, 4))
        self._create_number_spinner(end_cont, self.azimuth_end, 0, 360).pack(anchor=tk.W)
    ...
    def _setup_multiple_sectors_ui(self):
        """Multiple sectors - with custom sectors on the RIGHT side"""
        ...
        # RIGHT SIDE: Custom sectors or preset content
    ...
    def _setup_custom_sectors_mode(self):
        """Custom sectors - PERFECTLY CENTERED with compact yellow box - PERFORMANCE OPTIMIZED"""
        ...
        # Sectors list - 完美居中对齐 (store reference for updates)
